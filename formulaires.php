<?php
	...
	if(isset($_POST["envoyeri"])){
		...
		//$extentionFile = $_FILES["fileToUpload"]["type"]; fonctionnera pas
		$extentionFile = explode('.',$namefile);
		//echo $split[1];
		...
		function verif_null($var){
			...
			//si valeur de variable différente de :
		    	if($var!="" && $var!="Inscrivez ici votre prénom" && $var!="Inscrivez ici votre nom"){
				//renvoie la valeur de $var pour être utilisée ailleur
		    		return $var;
		    		}
		}

		if(verif_null($nomi) && verif_null($prenomi) && $extentionFile[1]=="pdf"){
			//$GLOBALS["hider"]="tohide";
			//$html = preg_replace('#<form id="exe3">(.*?)</form>#', '', $html);
			$message = "Bonjour ".$optionSelect." ".$nomi." ".$prenomi."<br>Nom du fichier à uploader: ".$namefile." d'extention : ".$extentionFile[1];
		}
		else{$message ="Vueillez remplir les champs, le fichier uploadé doir être un pdf";}
    ...
    <body>
        <div class="container">
			...
			<form  enctype="multipart/form-data" action="#" method="post" id="exe3" 
						<?php 
						if(isset($_POST["envoyeri"]) && $extentionFile[1]=="pdf"){ 
